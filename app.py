...
class PaymentSchema(ma.Schema):
  class Meta:
    fields = ('id', 'username', 'card_number', 'cvv_number', 'expiration_date')

# Init Schema
product_schema = PaymentSchema()
products_schema = PaymentSchema(many=True)
...
def make_payment():
  ...
  new_transaction = Payment(username, card_number, cvv_number, expiration_date)
  db.session.add(new_transaction)
  db.session.commit()

  return product_schema.jsonify(new_transaction)

# Get all transactions
@app.route('/transactions', methods=['GET'])
def get_transactions():
  all_transactions = Payment.query.all()
  result = products_schema.dump(all_transactions)
  return jsonify(result)

# Get single product (Not part of the task given)
@app.route('/transactions/<id>', methods=['GET'])
def get_transaction(id):
  transaction = Payment.query.get(id)
  return product_schema.jsonify(transaction)
